...
func (repo *UsersRepository) GetByLogin(ctx context.Context, login string) (*user.UserModel, error) {
	var au user.UserModel

	query := `SELECT id, login, password FROM users WHERE login = $1`

	if err := repo.db.QueryRowContext(ctx, query, login).Scan(&au.ID, &au.Login, &au.Password); err != nil {
